        ...
        async function fetchEvents() {
            try {
                ...
                const transformedEvents = data.map((event) => {
                    ...
                    const startHours = startDate.getHours() * 100 + startDate.getMinutes();
                    const endHours = endDate.getHours() * 100 + endDate.getMinutes();

                    return new Map([
                        ['startFull', startDate],
                        ['date', new Date(startDate.toDateString())], // Only the date part
                        ['start', startHours],
                        ['end', endHours],
