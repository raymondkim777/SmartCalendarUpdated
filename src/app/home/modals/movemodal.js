    ...
    return(
        <div style={{top: '50%', left: '50%', transform: 'translate(-50%, -43%)'}} className={`flex flex-col items-center z-10 absolute w-96 lg:w-[32rem] xl:w-[36rem] object-center top-60 h-fit mr-2 pt-1 rounded-lg border border-neutral-400 bg-stone-50`}>
            <div className='flex flex-row w-full h-10 items-center justify-between pl-4 pr-2 space-x-3'>
                <h1 className="text-lg text-nowrap truncate leading-6 text-gray-600 font-semibold">({methodList[moveDetails.get('type')]}) {moveDetails.get('name')}</h1>
            ...
            <div className='flex flex-col w-full h-fit max-h-64 lg:max-h-[32rem] xl:max-h-[40rem] items-center justify-start py-4 px-4 space-y-4 overflow-y-auto'>
                ...
                <div className='flex flex-col w-full h-fit items-center justify-start space-y-4'>
                    <div className='flex flex-row w-full h-fit items-center justify-start space-x-4 px-2'>
                        <span className='text-base leading-6 text-gray-600 font-normal'>Transportation Type: </span>
                        <span className='text-base text-wrap leading-6 text-gray-600 font-normal'>{methodList[moveDetails.get('type')]} </span>
                    </div>
                </div>
                ...
                <div className='flex flex-row w-full h-12 items-center justify-start space-x-4 px-2'>
                    ...
                    <div className='flex flex-col grow h-12 items-start justify-center'>
                        <span className='text-base leading-6 text-gray-600 font-normal'>{moveDetails.get('locations')} - ({formatDate(moveDetails.get('start'))})</span>
                        <span className='text-base leading-6 text-gray-600 font-normal'>{moveDetails.get('locatione')} - ({formatDate(moveDetails.get('end'))})</span>
                    </div>
                </div>
                {
                    moveDetails.get('description') != '' && 
                    <div className='flex flex-row w-full h-fit items-center justify-start space-x-4 px-2'>
                        <span className='text-base leading-6 text-gray-600 font-normal'>Description: </span>
                        <span className='text-base leading-6 text-gray-600 font-normal'>{moveDetails.get('description')} </span>
                    </div>
                }
                {/* Location */}
                <div className='flex flex-col w-full h-fit items-center justify-start space-y-4'>
