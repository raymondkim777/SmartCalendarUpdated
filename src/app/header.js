...
const Header = () => {
    ...
    const redirectWindow = window.open(googleLoginUrl, '_self');
    if (redirectWindow) {
        // Logic to handle token and set state after login can be added here
        // Example: Fetch user profile with the returned token
    }
            ...
            <div className='relative flex flex-row w-full max-w-7xl h-full px-6 items-center'>
                ...
                <div className='flex flex-row w-fit h-full items-center justify-center pl-2 space-x-3'>
                    ...
                    <div onClick={()=>setShowAccountModal(true)} className="flex w-12 h-12 items-center justify-center rounded-full overflow-hidden bg-gray-300 transition-all duration-300 hover:opacity-70 hover:cursor-pointer">
                        {loginStatus ? 
                            <img className="object-cover w-12 h-12 " src={'/example_profile.jpg'} /> :
                ...
                <div className={`flex flex-col items-center z-10 ${showAccountModal && !loginStatus ? '' : 'hidden'} absolute top-24 right-2 w-60 h-fit mr-2 pt-1 rounded-lg border border-neutral-400 bg-stone-50`}>
                    ...
                    <div onClick={()=>handleLogin()} className='flex flex-row w-full h-12 items-center justify-center space-x-3 px-4 rounded-b-md hover:cursor-pointer hover:bg-gray-200 active:bg-gray-300 transition-all duration-300'>
                ...
                {/* <Modal open={showAccountModal} overflow onClose={()=>setShowAccountModal(false)}>
                    <Modal.Header>
                    ...
                    <Modal.Footer>
                        <Button onClick={()=>setShowAccountModal(true)} appearance="primary">
                            Ok
                        </Button>
                        <Button onClick={()=>setShowAccountModal(true)} appearance="subtle">
                            Cancel
                        </Button>
                    </Modal.Footer>
                </Modal> */}
